def output(event):
     s = ent.get()          #input значений
     if s == "1":
          tex.delete(1.0,END)      #очистка экрана
...
ent.grid(row=0,column=0,padx=20)        #выравнивание, сколько надо отступить от начала ячейки
but.grid(row=0,column=1)
tex.grid(row=0,column=2,padx=20,pady=10)
 
but.bind("<Button-1>",output)      #связывает событие с каким-то действием
