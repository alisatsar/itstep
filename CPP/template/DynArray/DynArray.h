...
DynArray<T>& DynArray<T>::operator=(DynArray<T> const& dynArray)
{
	if (this != &dynArray)
	{
		//после создания move construct можно написать так
...
template<class T>
std::ostream& operator<<(std::ostream& stream, DynArray<T> const& dynArray)
{
	for (int i = 0; i < dynArray.m_count; i++)
	{
		stream << dynArray.m_data[i] << " ";
	}
	return stream;
}
