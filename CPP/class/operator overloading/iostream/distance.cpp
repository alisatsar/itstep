...
std::ostream& operator<<(std::ostream& stream, Distance const& d)		//ссылка для того, чтобы не копировать а изменить
{
...
{
	char buffer[20];
	stream.getline(buffer, 19);										//getline читает строку до \n или 19 символов
	char* begin = buffer;
	int feet = strtol(begin, &begin, 10);							//strtol Делает из строки число					
	if (*begin != '\'')
	{
		std::cout << "Wrong format\n";
		exit(-1);													// exit код возврата кроме нуля ошибка
	}
...
Distance operator+(int feet, Distance distance)
{
	distance.m_feet += feet;
	return distance;
}
